...
const banner = computed(() => {
  const queries = [page.sources, page.jobs, page.health];
  if (queries.some((query) => query.data.value === undefined && query.isError.value)) return "unavailable";
  if (queries.some((query) => query.data.value !== undefined && (query.isError.value || query.isStale.value))) return "stale";
  if (queries.some((query) => query.data.value?.meta.status === "unavailable")) return "unavailable";
  if (queries.some((query) => query.data.value?.meta.status === "partial")) return "partial";
  return page.health.data.value?.summary.level ?? null;
});
...
function bannerLabel(kind: string | null) {
  const allowed = ["healthy", "busy", "paused", "degraded", "blocked", "partial", "stale", "unavailable"];
  return t(`localStatus.banner.${kind !== null && allowed.includes(kind) ? kind : "unknown"}`);
}
...
<template>
  <section ref="root" data-testid="local-status-view" class="w-full space-y-4 py-1">
    <div class="flex flex-wrap items-center justify-between gap-3">
      <span v-if="banner !== null && !initialPending && !fatal" data-testid="local-status-banner" role="status" aria-live="polite" class="rounded-full bg-amber-100 px-3 py-1 text-sm text-amber-900">
        {{ bannerLabel(banner) }}
      </span>
    ...
    <div v-if="pauseAllConfirmationOpen" data-testid="pause-all-dialog" role="dialog" aria-modal="true" :aria-label="t('localStatus.action.pauseAllConfirmTitle')" class="rounded-content border border-red-200 bg-red-50 p-5" @keydown.esc.prevent="cancelPauseAllConfirmation">

...
public struct TokenActivityCalendarPresentation: Equatable, Sendable {
    ...
    public init(_ activity: TokenActivityPresentation) {
        ...
        let positiveValues = activity.days.compactMap(\.tokens).filter { $0 > 0 }.sorted()
        let thresholds = Self.thresholds(positiveValues)
        let calendarDays = activity.days.map { day in
            TokenActivityCalendarDay(
                day: day,
                intensity: Self.intensity(tokens: day.tokens, thresholds: thresholds)
            )
        }
    ...
    private static func thresholds(
        _ values: [Int64]
    ) -> (low: Int64, medium: Int64, high: Int64)? {
        guard !values.isEmpty else { return nil }
        func value(at fraction: Double) -> Int64 {
            values[Int(Double(values.count - 1) * fraction)]
        }
        return (value(at: 0.25), value(at: 0.5), value(at: 0.75))
    }

    private static func intensity(
        tokens: Int64?,
        thresholds: (low: Int64, medium: Int64, high: Int64)?
    ) -> TokenActivityIntensity {
        guard let tokens else { return .unknown }

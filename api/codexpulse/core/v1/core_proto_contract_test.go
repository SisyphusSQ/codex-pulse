...
func TestCoreProtoPreservesPresenceAndContentFreeErrors(t *testing.T) {
	...
	for _, pattern := range []string{
		...
		`(?s)message ContractsResponse\s*\{.*string dashboard_summary_version\s*=\s*10\s*;.*string codex_pro_tier_version\s*=\s*11\s*;.*string codex_subscription_accounts_version\s*=\s*12\s*;.*string provider_control_version\s*=\s*13\s*;`,

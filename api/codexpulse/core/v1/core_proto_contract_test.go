...
func TestCoreProtoPreservesPresenceAndContentFreeErrors(t *testing.T) {
	...
	for _, pattern := range []string{
		...
		`(?s)message ActivityDistribution\s*\{.*string timeline_granularity\s*=\s*1\s*;.*repeated ActivityTimelinePoint timeline\s*=\s*2\s*;.*repeated ActivityWeekdayHourPoint weekday_hours\s*=\s*3\s*;`,

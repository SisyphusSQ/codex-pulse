...
type runtimeControlBindingCommand interface {
...
func (service *Service) UpdateSettings(
	...
	if command == nil {
		return SettingsUpdateReceipt{}, newBindingFailure(ErrBindingService)
	}
	if !validSettingsUpdateRequest(request) {
		return SettingsUpdateReceipt{}, newBindingFailure(basequery.NewValidationFailure("settings", nil))
	}
	return bindingCall(func() (SettingsUpdateReceipt, error) {
...
func (service *Service) PlanHomeSwitch(
	...
	command := service.runtimeControlsCommand()
	if command == nil {
		return HomeSwitchPlanReceipt{}, newBindingFailure(ErrBindingService)
	}
	if !filepath.IsAbs(request.TargetPath) || filepath.Clean(request.TargetPath) != request.TargetPath {
		return HomeSwitchPlanReceipt{}, newBindingFailure(basequery.NewValidationFailure("targetPath", nil))
	}
	if !validHomeSwitchStrategy(request.Strategy) {
		return HomeSwitchPlanReceipt{}, newBindingFailure(basequery.NewValidationFailure("strategy", nil))
	}
	return bindingCall(func() (HomeSwitchPlanReceipt, error) {
...
func (service *Service) ConfirmHomeSwitch(ctx context.Context) (HomeSwitchReceipt, error) {
	command := service.runtimeControlsCommand()
	if command == nil {
		return HomeSwitchReceipt{}, newBindingFailure(ErrBindingService)
	}
	return bindingCall(func() (HomeSwitchReceipt, error) { return command.ConfirmHomeSwitch(ctx) })
}
...
func (service *Service) RecoverHomeSwitch(ctx context.Context) (HomeSwitchReceipt, error) {
	...
	return bindingCall(func() (HomeSwitchReceipt, error) { return command.RecoverHomeSwitch(ctx) })
}
...
func (service *Service) RunRuntimeAction(
	...
	command := service.runtimeControlsCommand()
	if command == nil {
		return RuntimeActionReceipt{}, newBindingFailure(ErrBindingService)
	}
	if !validRuntimeAction(action) {
		return RuntimeActionReceipt{}, newBindingFailure(basequery.NewValidationFailure("action", nil))
	}
	return bindingCall(func() (RuntimeActionReceipt, error) {
...
func (service *Service) AnalyzeSessionIndexRepair(ctx context.Context) (RepairDryRunReceipt, error) {
	command := service.runtimeControlsCommand()
	if command == nil {
		return RepairDryRunReceipt{}, newBindingFailure(ErrBindingService)
	}
	return bindingCall(func() (RepairDryRunReceipt, error) {
		return command.AnalyzeSessionIndexRepair(ctx)
	})
}

func (service *Service) runtimeControlsCommand() runtimeControlBindingCommand {
	if service == nil {
		return nil
	}

...
func expiredJobRuns(database *gorm.DB, cutoffMS int64) *gorm.DB {
	return expiredJobRunsBase(database, cutoffMS).
		Where("state IN ?", []string{"succeeded", "failed", "cancelled"})
}

func expiredJobRunsForState(database *gorm.DB, cutoffMS int64, state string) *gorm.DB {
	return expiredJobRunsBase(database, cutoffMS).Where("state = ?", state)
}

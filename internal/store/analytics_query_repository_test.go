...
func TestUsageCostRangeBuildsTokenAndDistinctSessionActivityDistribution(t *testing.T) {
	...
	if snapshot.ActivityDistribution.TimelineBucketMinutes != 180 {
		t.Fatalf(
			"timeline bucket minutes = %d",
			snapshot.ActivityDistribution.TimelineBucketMinutes,
		)
	}
	wantTimeline := []UsageActivityTimelinePoint{
		{
			BucketStartMS: mustParseCostTime(t, "2026-07-20T00:00:00Z"),
			BucketEndMS:   mustParseCostTime(t, "2026-07-20T03:00:00Z"),
			TotalTokens:   pointerTo(int64(60)), SessionCount: 2,
		},
		{
			BucketStartMS: mustParseCostTime(t, "2026-07-21T00:00:00Z"),
			BucketEndMS:   mustParseCostTime(t, "2026-07-21T03:00:00Z"),
			TotalTokens:   pointerTo(int64(40)), SessionCount: 1,
		},
		{
			BucketStartMS: mustParseCostTime(t, "2026-07-27T00:00:00Z"),
			BucketEndMS:   mustParseCostTime(t, "2026-07-27T03:00:00Z"),
...
func TestValidateAnalyticsRangeRejectsInvalidActivityBuckets(t *testing.T) {
	t.Parallel()

	base := AnalyticsRange{
		ReportingTimezone: "UTC",
		StartAtMS:         mustParseCostTime(t, "2026-07-20T00:00:00Z"),
		EndAtMS:           mustParseCostTime(t, "2026-07-21T00:00:00Z"),
		Granularity:       AnalyticsGranularityHour,
	}
	for _, filter := range []AnalyticsRange{
		func() AnalyticsRange {
			value := base
			value.ActivityBucketMinutes = 120
			return value
		}(),
		func() AnalyticsRange {
			value := base
			value.IncludeActivityDistribution = true
			value.ActivityBucketMinutes = 90
			return value
		}(),
	} {
		if _, err := validateAnalyticsRange(filter); !errors.Is(err, ErrInvalidRecord) {
			t.Fatalf(
				"validateAnalyticsRange(%#v) error = %v, want ErrInvalidRecord",
				filter,
				err,
			)
		}

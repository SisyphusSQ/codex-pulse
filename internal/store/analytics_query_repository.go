...
func loadUsageActivityDistribution(
	...
	bucketMinutes := resolvedActivityBucketMinutes(filter)
	...
	for _, fact := range facts {
		...
		bucket := analyticsActivityBucketStart(
			fact.observedAtMS,
			filter,
			location,
			bucketMinutes,
	...
	distribution := &UsageActivityDistribution{
		TimelineGranularity:   AnalyticsGranularityHour,
...
func validateAnalyticsRange(filter AnalyticsRange) (*time.Location, error) {
	...
	if filter.IncludeActivityDistribution &&
		!validActivityBucketMinutes(resolvedActivityBucketMinutes(filter)) {
		return nil, invalidRecord("analytics activity bucket is invalid")
	}
...
func validActivityBucketMinutes(value int) bool {
	switch value {
	case 60, 120, 180, 360, 720, 1_440:
		return true
	default:
		return false
	}
...
func analyticsActivityBucketStart(
	observedAtMS int64,
	filter AnalyticsRange,
	location *time.Location,
	bucketMinutes int,
	...
	if bucketMinutes >= 1_440 {
		bucket := localDayBucketStart(observedAtMS, location)
		if bucket < filter.StartAtMS {
			return filter.StartAtMS
		}
		return bucket
	}
	bucketMS := int64(bucketMinutes) * int64(time.Minute/time.Millisecond)
	offset := observedAtMS - filter.StartAtMS
	if offset <= 0 {
		return filter.StartAtMS
	}
	return filter.StartAtMS + offset/bucketMS*bucketMS
}

func analyticsActivityBucketEnd(
	bucketStartMS int64,
	filter AnalyticsRange,
	location *time.Location,
	bucketMinutes int,
	...
	if bucketMinutes >= 1_440 {
		...
		endAtMS = bucketStartMS +
			int64(bucketMinutes)*int64(time.Minute/time.Millisecond)
	}
	if endAtMS > filter.EndAtMS {
		return filter.EndAtMS
	}

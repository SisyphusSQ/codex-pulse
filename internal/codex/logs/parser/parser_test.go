...
func TestStreamParserParsesCompleteLinesAndRetainsIncompleteTail(t *testing.T) {
	...
	parser, err := NewStreamParser(ParserConfig{
		SourceKind: source.SourceKindSession, StartOffset: startOffset, MaxLineBytes: 4096,
	})
	if err != nil {
	...
	if first.Events[0].SessionMeta.SourceKind != source.SourceKindSession {
...
func TestStreamParserChunkingIsDeterministic(t *testing.T) {
	...
	if _, err := NewStreamParser(ParserConfig{
		SourceKind: source.SourceKindSession, StartOffset: oneChunk.CommittableOffset, Seed: oneChunk.NextSeed,
	}); err != nil {
...
func TestStreamParserSkipsOversizeLineAndAdvancesOnlyThroughCompleteLines(t *testing.T) {
	...
	parser, err := NewStreamParser(ParserConfig{
		SourceKind: source.SourceKindArchivedSession, MaxLineBytes: 8,
	})
	if err != nil {
		t.Fatalf("NewStreamParser() error = %v", err)
	}
...
func TestStreamParserCarriesArchivedSourceKind(t *testing.T) {
	...
	parser, err := NewStreamParser(ParserConfig{SourceKind: source.SourceKindArchivedSession})
	if err != nil {
		t.Fatalf("NewStreamParser() error = %v", err)
	}
	...
	if len(result.Events) != 1 || result.Events[0].SessionMeta == nil ||
		result.Events[0].SessionMeta.SourceKind != source.SourceKindArchivedSession {
...
func TestStreamParserRejectsInvalidConfigAndNonContiguousChunksWithoutMutation(t *testing.T) {
	...
	for _, testCase := range []struct {
		...
		{name: "session index", config: ParserConfig{SourceKind: source.SourceKindSessionIndex}, err: ErrUnsupportedParserSource},
		{name: "unknown source", config: ParserConfig{SourceKind: source.SourceKind("future")}, err: ErrUnsupportedParserSource},
		{name: "negative offset", config: ParserConfig{SourceKind: source.SourceKindSession, StartOffset: -1}, err: ErrInvalidParserConfig},
		{name: "negative limit", config: ParserConfig{SourceKind: source.SourceKindSession, MaxLineBytes: -1}, err: ErrInvalidParserConfig},
		{name: "over hard limit", config: ParserConfig{SourceKind: source.SourceKindSession, MaxLineBytes: MaxSupportedLineBytes + 1}, err: ErrInvalidParserConfig},
		{name: "nonzero offset without seed", config: ParserConfig{SourceKind: source.SourceKindSession, StartOffset: 10}, err: ErrInvalidParserSeed},
		{name: "seed at origin", config: ParserConfig{SourceKind: source.SourceKindSession, Seed: testParserSeed("session-1")}, err: ErrInvalidParserSeed},
		{name: "duplicate seeded turn", config: ParserConfig{
			SourceKind: source.SourceKindSession, StartOffset: 10,
		...
		{name: "seeded source mismatch", config: ParserConfig{SourceKind: source.SourceKindArchivedSession, StartOffset: 10, Seed: testParserSeed("session-1")}, err: ErrInvalidParserSeed},
		...
		{name: "pending context after terminal", config: ParserConfig{
			SourceKind: source.SourceKindSession, StartOffset: 20,
			Seed: &ParserSeed{Session: testSessionFact("session-1"), PendingTurns: []PendingTurnSeed{{
				TurnID: "turn-1",
	...
	preSession, err := NewStreamParser(ParserConfig{
		SourceKind: source.SourceKindSession, StartOffset: 10, Seed: &ParserSeed{},
	})
	if err != nil {
	...
	parser, err := NewStreamParser(ParserConfig{
		SourceKind: source.SourceKindSession, StartOffset: 10, MaxLineBytes: 1024,
...
func TestStreamParserHydratesNonzeroOffsetAndFreezesSeededUsage(t *testing.T) {
	...
	parser, err := NewStreamParser(ParserConfig{
		SourceKind: source.SourceKindSession, StartOffset: 100, MaxLineBytes: 4096, Seed: seed,
	})
	if err != nil {
		t.Fatalf("NewStreamParser() error = %v", err)
	}
...
func TestStreamParserPersistsPendingCheckpointAcrossRestart(t *testing.T) {
	...
	parser, err := NewStreamParser(ParserConfig{SourceKind: source.SourceKindSession, StartOffset: startOffset, Seed: seed})
	if err != nil {
		t.Fatalf("NewStreamParser() error = %v", err)
	}
	...
	restarted, err := NewStreamParser(ParserConfig{
		SourceKind: source.SourceKindSession, StartOffset: first.ReadOffset, Seed: first.NextSeed,
	})
	if err != nil {
...
func TestStreamParserCheckpointKeepsOrphanPendingLiveAcrossRestart(t *testing.T) {
	...
	parser, err := NewStreamParser(ParserConfig{SourceKind: source.SourceKindSession})
	if err != nil {
		t.Fatalf("NewStreamParser() error = %v", err)
	}
	...
	restarted, err := NewStreamParser(ParserConfig{
		SourceKind: source.SourceKindSession, StartOffset: continued.CommittableOffset,
...
func TestStreamParserCheckpointPreservesOpenContextAndClosedReplay(t *testing.T) {
	...
	continuousParser, err := NewStreamParser(ParserConfig{SourceKind: source.SourceKindSession})
	if err != nil {
		t.Fatalf("NewStreamParser() error = %v", err)
	}
	...
	restartedParser, err := NewStreamParser(ParserConfig{
		SourceKind: source.SourceKindSession, StartOffset: prefixResult.CommittableOffset, Seed: prefixResult.NextSeed,
	})
	if err != nil {
		t.Fatalf("NewStreamParser(restarted) error = %v", err)
	}
	...
	closedRestart, err := NewStreamParser(ParserConfig{
		SourceKind: source.SourceKindSession, StartOffset: closed.CommittableOffset, Seed: closed.NextSeed,
	})
	if err != nil {
...
func TestStreamParserGeneratedCheckpointFreezesUsageAcrossRestart(t *testing.T) {
	...
	restarted, err := NewStreamParser(ParserConfig{
		SourceKind: source.SourceKindSession, StartOffset: first.CommittableOffset, Seed: first.NextSeed,
	})
	if err != nil {
		t.Fatalf("NewStreamParser(restarted) error = %v", err)
	}
...
func testSessionFact(sessionID string) *SessionMetaFact {
	return &SessionMetaFact{
		SessionID: sessionID, RootSessionID: sessionID, SourceKind: source.SourceKindSession,

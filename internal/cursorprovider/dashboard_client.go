...
var ErrDashboardProtocol = errors.New("cursor dashboard protocol is unavailable")
...
func (client *DashboardClient) call(ctx context.Context, method string, payload []byte) ([]byte, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("%w: request failed", ErrDashboardProtocol)
	}
	...
	if response.StatusCode != http.StatusOK || !strings.HasPrefix(response.Header.Get("Content-Type"), "application/proto") {
		return nil, fmt.Errorf("%w: unexpected response status %d", ErrDashboardProtocol, response.StatusCode)
	}
	limited := io.LimitReader(response.Body, maxDashboardResponseBytes+1)
	body, err := io.ReadAll(limited)
	if err != nil || len(body) > maxDashboardResponseBytes {
		return nil, fmt.Errorf("%w: invalid response body", ErrDashboardProtocol)
	}
	return body, nil
}

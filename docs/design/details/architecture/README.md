...
`wails-bindings-v1` 当前只允许以下 query：`Bootstrap`、`Contracts`、`UsageCost`、`ListSessions`、`SessionDetail`、`ListProjects`、`ProjectDetail`、`QuotaCurrent`、`ListSources`、`Source`、`ListJobs`、`Job`、`ListHealth`、`Health`、`Settings`，以及唯一 command `RequestQuotaRefresh`。`Contracts` 返回 binding/query/业务 contract 版本、完整 method allowlist、精确 command allowlist 和一个 typed error exemplar。refresh command 只接收 `quota/reset_credits`，返回不含 source instance、scope、claim、revision 的 receipt；现有 Preferences、Schedule 与 Codex Home lifecycle mutator 仍保持不可绑定。新增 exported method 必须先让 exact allowlist contract test 失败，再通过独立 Issue 和 review。

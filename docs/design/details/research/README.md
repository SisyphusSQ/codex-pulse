...
这是一份调研快照，不代表相关项目或 Wails3 的当前最新状态。进入实现前，容易变化的版本、许可证、接口和发布能力需要重新读回验证。
...
- 2026-07-03：阅读 Codex Runway README、截图和 Swift 源码结构，判断可复刻能力和 Go + Wails3 方向。
...
- 窄 Popover 只适合快速查看，分析应进入 Wails3 主窗口。
- 内部接口必须显示来源、时间和失败状态。
...
- Go + Wails3 适合“Go 本地数据层 + 富前端工作台”；具体 Wails3 版本需要实现前重新验证并固定。

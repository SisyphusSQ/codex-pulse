# Codex Pulse Design Details

模块设计按职责拆分，避免产品、数据、调度和发布约束在同一篇文档中混杂。

- [Product](product/README.md)
- [Architecture](architecture/README.md)
- [Agent Providers and Cursor](providers/README.md)
- [Native macOS Client and Go Helper Refactor](native-macos-client/README.md)
- [Data Model and Indexing](data-model/README.md)
- [Quota](quota/README.md)
- [Scheduling and Bootstrap](scheduling-and-bootstrap/README.md)
- [Updates and Release](updates-and-release/README.md)
- [Observability and Data Health](observability/README.md)
- [Research](research/README.md)

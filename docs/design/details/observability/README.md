...
TOO-299 另冻结 core bootstrap 和前台公共查询的性能门禁。真实 Home cold bootstrap 至少三轮，每轮由 `Bootstrap Runtime.RunSlice` 直接连续执行并独立通过后再以 nearest-rank 聚合；FirstScreenReady `<=60s`、FullHistoryReady `<=20m`、bootstrap throughput `>=4MiB/s`、内部 peak RSS `<=1GiB`、WAL `<=512MiB`。同一完成 Store 上各采样 30 次，Session/Usage p95 `<=50ms`、Project `<=200ms`、Quota `<=750ms`、Session→Project→Usage→Quota 顺序组成的 Popover backend suite `<=1s`。runner 不经过 production scheduler/cron，因此该 wall time 不冒充 packaged app 启动；Popover suite 同样不包含 WebView paint 或 AppKit 视觉时间，controller adapter dispatch 只以独立 allocation benchmark 记录。

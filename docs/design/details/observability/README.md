...
TOO-282 将上述进程内 projection 通过只读 `HealthProjection` Core RPC 交给 client cache。DTO 只包含 `hasValue/stale/failure/evaluatedAt/level/primary/components` 以及七组件的有限 evidence/reason/impact/protection/recovery action；缺失 reader、非法组件数、重复 component、非法 enum 或 primary/level 不一致均 fail closed 为 typed RPC error，不回退到持久事件数量、日志或 client 推断。health invalidation、wake、runtime ready 与 foreground 只使 cache 失效并重取；5 秒 active polling 覆盖 evaluator 周期投影更新，event payload 不携带业务状态。
...
TOO-283 将 Data Health 固定为“本机状态”下的二级页面，不新增主导航项。只读 `DataHealth(evaluatedAtMS)` Core RPC 从既有 GORM-first `MetricsSnapshot` 取得精确 24 小时半开窗口，并映射 runtime CPU/RSS、DB/WAL/磁盘、live/backfill queue、scheduler/job/source 聚合；Detailed 模式最多 17,280 个原始点由 Go 确定性压缩为最多 289 个趋势点，保留最新与窗口内最早证据，native client 不自行读取 SQLite 或从图表推断健康。同一 query 另以 Store typed `CurrentOnly` Job page 优先读取 queued/running 与 `resume_consumed_by_job_id IS NULL` 的 interrupted，再以 active Health page 读取 open 事实；各自剩余的 12 项显示预算只补充窗口内 terminal/resolved 历史。已被 resume child 消费的 interrupted 不再是 current，较新的终态记录不能挤掉当前/open 事实，窗口外历史在 Go 中裁剪。资源/活动读取以 `DataHealth.evaluatedAtMs` 标记，Health Projection 显示自己的 `evaluatedAtMs`；两者不被伪装成同一数据库事务。
...
TOO-299 另冻结 core bootstrap 和 client-facing 公共查询的性能门禁。真实 Home cold bootstrap 至少三轮，每轮由 `Bootstrap Runtime.RunSlice` 直接连续执行并独立通过后再以 nearest-rank 聚合；FirstScreenReady `<=60s`、FullHistoryReady `<=20m`、bootstrap throughput `>=4MiB/s`、内部 peak RSS `<=1GiB`、WAL `<=512MiB`。同一完成 Store 上各采样 30 次，Session/Usage p95 `<=50ms`、Project `<=200ms`、Quota `<=750ms`、Session→Project→Usage→Quota 顺序组成的 Popover backend suite `<=1s`。runner 不经过 production scheduler/cron，因此该 wall time 不冒充 packaged app 启动；Popover suite 同样不包含 native view render 或 AppKit 视觉时间，RPC adapter dispatch 只以独立 allocation benchmark 记录。

提交版结果见 [`docs/test/m11-e3.md`](../../../test/m11-e3.md)：Apple M4 Pro 上约 6.56GB 真实只读 Home 三轮全部通过，FirstScreenReady p95 `38.367s`、FullHistoryReady p95 `919.556s`、内部 peak RSS 最大约 `242MiB`、WAL 最大约 `63.85MiB`。真实性、隐私、source 前后只读与隔离 cleanup 必须和数值门槛同时通过；`.artifacts/runs` 原始进度、time 与 JSON 继续保持本地忽略。

同卡 support matrix 用 5×20 次真实 `grown` live ingest、1000 行 retention 与 query invalidation benchmark补齐增量/事件门槛，并用三轮 packaged App 的独立离线 Home 验证 durable bootstrap 已完成后的 Tray idle：平均 CPU 最坏 `0.480%`、peak RSS 约 `129.72MiB`、WAL 约 `0.648MiB`。append 必须读回 canonical completed fact；idle runner 必须读回 bootstrap `succeeded`、active scheduler task `0` 和隔离 cleanup；只看 checkpoint 或低 CPU 不能标记 PASS。冻结 evidence 还需绑定运行前 tracked diff、untracked tree、稳定 support source 与 packaged executable SHA。

TOO-284 将该目标收敛为可重复的 macOS arm64 validation matrix：真实 application collector 每轮同时执行 gopsutil process probe、SQLite/WAL/磁盘与 runnable queue probe、24 小时 MetricsSnapshot query 和 maintenance writer，并按 Detailed 5 秒 cadence 报告 `duty_pct`。机械门槛固定为 duty `<0.5%`、query `<50ms`、RSS `<512MiB`、WAL `<256MiB`；每轮 100 次、重复 5 次，任一缺少 metric 或越界都 fail closed。idle/live/backfill/query/cleanup 另以 BSD time 记录阶段 process envelope，但该 envelope 包含 Go test runner，不能冒充产品常驻 RSS；产品数值只使用 collector 持久的 typed sample。

同一故障矩阵只在临时目录、fake transport/adapter 与专用 child process 注入 permission、disk full/read-only、SQLite lock、malformed row、network、sleep/wake 和 process interruption。它不修改真实 Home、应用数据库、权限、磁盘、网络或系统睡眠。提交版结果见 [`docs/test/m8-e6.md`](../../../test/m8-e6.md)；原始命令与 time 输出只保存在忽略的 `.artifacts/runs`。当前实测固定 macOS 15.0 deployment target，但运行 OS 必须单独记录，不能用较新系统的通过结果冒充实际 macOS 15 runtime。

...
提交版结果见 [`docs/test/m11-e3.md`](../../../test/m11-e3.md)：Apple M4 Pro 上约 6.56GB 真实只读 Home 三轮全部通过，FirstScreenReady p95 `38.367s`、FullHistoryReady p95 `919.556s`、内部 peak RSS 最大约 `242MiB`、WAL 最大约 `63.85MiB`。真实性、隐私、source 前后只读与隔离 cleanup 必须和数值门槛同时通过；`.agents/runs` 原始进度、time 与 JSON 继续保持本地忽略。

...
TOO-295 将上述原语接入 application startup gate。migration failure 关闭当前 Store 后只保留 content-free recovery snapshot；普通 Store/Repository/updater service graph 不启动。restore 选择仅限私有 `0700` backup 目录内经 `O_NOFOLLOW` 打开的 regular `0600` DB，prepare 时冻结 SHA-256，confirm 时从新 descriptor 复制并复核摘要；恢复到 working DB 后重新执行 v1-v15 migration、完整 schema verify 和 builtin pricing readback，再用 modernc Backup 固化 ready DB。canonical 切换前以 SQLite Online Backup 保留包含 committed WAL 的失败库，随后用 Darwin 原子文件交换发布 ready DB；pre-commit 同步失败原子回滚，post-commit 同步异常显式返回 committed warning，因此不会覆盖唯一副本或留下 canonical path 缺口。恢复成功仍需新进程再次通过 startup gate。

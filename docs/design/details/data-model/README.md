...
| `event_msg.token_count` | session cumulative 与 last-turn nullable input/cached/output/reasoning counters、optional context window；独立解析 `rate_limits.limit_id/primary/secondary/plan_type` 与 window 的 used percent、时长、reset |
...
原始 `sessions.initial_cwd`、`turns.cwd/model` 与 `session_current.current_cwd/current_model` 仍是 parser 观察到的本机事实，不会因规则变化被改写。安全 attribution projection 只暴露 stable ID、受限 display、固定 confidence/source/reason 和 rule version；它没有 `root_path`、`initial_cwd`、`cwd`、`current_cwd` 或 raw model 字段。`internal/core` 与 Core RPC 只能消费该安全 projection，不能直接序列化 core Store record。需要展示原始路径时必须另设受控、本机、显式 reveal capability，不得扩张 Protobuf DTO。
...
3. rule v2 在 Store 私有 adapter 中先识别本机项目路径，再交给纯 attribution 层生成安全 identity。存在 `.git` 的普通仓库会归一到最近仓库根；linked worktree 通过 `.git` 指向的 `commondir` 归并到主 working tree。同一仓库的不同 linked worktree 因此共享 project identity，原始 cwd、`.git` 路径和 common directory 都不会进入 RPC。
4. 形如 `Codex/YYYY-MM-DD/<session>` 的合法日期临时工作区不生成项目 identity，其 Session 和 Token 仍计入会话、趋势与总量，在项目维度统一显示为“其他”。相似目录名、非法日期或缺少 session component 的路径仍按普通目录处理。
5. 已移除的 `.codex/worktrees/<id>/<repository>` 无法再读取 `.git` 时，同名 Codex-managed worktree 使用受控的 repository component identity；若当前 Session 集合中只有一个同名 live Git repository，则回归该 canonical repository。普通同名目录或独立 clone 不使用 basename 合并。
6. 其余没有登记 root、也不是 Git 仓库的绝对 cwd 继续使用 `local-path-v1:<sha256(normalized-absolute-path)>` 建立 `medium/cwd_path_digest` 本机 identity；basename 仅作为受限 display。自动生成的 path-digest Project 不升级为跨 Session registered root，只在当前 Session 内让 initial cwd 与 nested turn/current 对齐。
...
Project Session/Model drill-down 是 schema v12 之上的只读 projection，不新增表、migration 或 index。`project_usage_daily` 仍是 Project item/daily/global totals 真相；精确 SessionCount 与 Project contribution pages 在同一 read snapshot 中从 final `turn_usage` + matching-source `turns` + safe `turn_attributions` + active-generation `turn_costs` 聚合，Session 展示字段只从 `session_attributions` 读取。Project/Model 缺失身份以 `unknown|confidence|source|reason` 保留；全量 Session groups 与全量 Model groups 分别作为 grouped subquery，由各自只返回一行的 NULL-preserving 外层 aggregate 对账 daily item totals，三方任一不一致都拒绝返回且不会把全量分组物化到 Go 内存。生产查询使用 GORM query builder，不引入 `.Raw`/`.Exec` 例外。
...
v3→v4 migration 在创建 attribution schema 后，使用同一个 GORM writer transaction 从既有 canonical Session/Turn facts 全量回填归因；只有 DDL、全部派生行、migration ledger 与 `user_version` 一起成功才提交。backfill 任一步失败会整体回滚到 v3，不能留下“schema 已是 v4、历史归因为空”的状态。后续正常 FactBatch 只在 `WriteUnit` 内登记 dirty Session；callback 全部成功后、commit 前按 Session ID 排序，每个 Session 最多刷新一次，避免 Indexer 逐 Fact 激活时重复扫描历史 Turns。schema v18 会把历史 Session/Turn attribution 统一重算为 rule v2，并级联清除依赖旧项目维度的可重建 cost generations；canonical Session、Turn、cwd、model 与 Token facts 不变。
...
- `quota_observation_receipts(observation_id, segment_observation_id, sample_sha256)`
- `quota_current(account_scope, window_kind, limit_id, observation_id, effective_used_percent, window_minutes, resets_at_ms, window_generation, selected_source, freshness_state, conflict_state, fresh_until_ms, last_success_at_ms, last_attempt_at_ms, rule_version, explanation_code, evaluated_at_ms)`
- `quota_arbitration_evidence(account_scope, window_kind, limit_id, observation_id, window_generation, disposition, reason, explanation_code)`

application schema v9 创建 `quota_observations` 与 `quota_observation_receipts`，v10 为 online source state/attempt 追加 typed failure metrics，v11 创建可重建 `quota_current` 与逐 observation 的 `quota_arbitration_evidence`，v19 为 observation 追加 nullable `limit_name`。v0.1 曾把 `account_scope` 固定为 `default`；这些行今天原样保留为 legacy/unassigned，不得回填给第一个发现的 ChatGPT 账号。schema v32 增加安装级 `codex_account_scope_key`、已见 `codex_account_scopes` 与单行 `codex_account_binding`：SQLite 只保存 64 位 lowercase hex HMAC scope 与 binding generation，不保存原始 `accountId`、token、JWT 或真实邮箱。新写入的 local JSONL observation 必须带 Session 和 durable `source_file_id` 且不得带 request；Session 后续删除时可由外键置空，但 physical source lineage 保留。当前在线 observation 必须带 request、当前 scope 与 binding generation，不得带 source file，Session 可空。`limit_id` 负责稳定区分通用与模型专属额度，`limit_name` 只负责展示，不能反向替代 identity。

schema v15 增加 `idx_quota_observations_projection(account_scope, window_kind, limit_id, last_observed_at_ms, observation_id)`，覆盖 quota projection 的逻辑窗口过滤与稳定排序，不改变 observation/evidence truth。projection 写入仍先在同一事务读取并仲裁完整 immutable history；写后读回改为精确校验 current、evidence 总数、changed rows 和 stale row absence，事务开始时已读回并比较的 unchanged evidence 不重复全表读取。changed/stale ID 均按 256 项分批，避免 SQLite variable limit；公共 `QuotaCurrent` / evidence reader 继续全量重算并 fail closed，性能优化不能把持久投影变成新的权威来源。

v11 的 current 主键是 `(account_scope, window_kind, limit_id)`；nullable selected tuple 保留“从未加载”与真实 `used_percent=0` 的差异。`window_generation` 等于通过校验的 `resets_at_ms`，由 STRICT CHECK 和 typed readback 双重约束，因此规则重算和补入旧 history 不会重编号当前代际。freshness 使用 `never_loaded/fresh/stale/expired_unknown/suspicious`，conflict 使用独立 `none/conflict` 轴；selected observation 外键为 `RESTRICT`，evidence 同时引用 current 复合键和原 observation。普通 projection replace/query/readback 全部使用 GORM；evidence 用 256 行固定批次写入。typed reader 在显式 GORM read transaction 的同一 SQLite snapshot 中读取完整 raw candidates 与 Wham source state，按 stored rule/evaluation 重算后，对 current 和完整 evidence 集合逐字段对账，因此并发 writer commit 不会制造混合快照，组合自洽篡改及 evidence 缺行/多行也会 fail closed。Local、`RecordQuotaFetch`、exact quota Wham `UpsertSourceState` 与全量 maintenance rebuild 在各自 writer transaction 内使用 repository 单次可信时钟重建受影响 projection；v10→v11 backfill 使用 migration runner 的同一次可信时钟。raw observation 与 attempt 时间只作为待校验事实，不作为 evaluation clock。`STRICT`、CHECK、复合 FK 和 index DDL 是集中 raw SQL 例外。

`resets_at_ms` 接受非负 Unix epoch milliseconds；`0` 只允许以 `suspicious/reset_not_future` 等非 accepted 事实存在。accepted observation 仍要求 reset 严格晚于最后观测时间，parser、Repository typed 校验与 STRICT CHECK 保持一致。

同一来源、physical source、Session/request、generation、limit、window、used、reset、plan 和 validity/reason 连续相同时，只更新 observation 的末次时间、末次 offset 与 sample count；任一语义变化、新 generation 或新 physical source 都新增 segment。每个原始 sample 的 ID 与 canonical fact digest 写入 `quota_observation_receipts`，因此 coalesce 不丢 exact replay 判定能力。`idx_quota_observations_current(account_scope, source, source_file_id, window_kind, limit_id, last_observed_at_ms DESC, observation_id DESC)` 支持后续 current 候选读取；partial unique `idx_quota_observations_source_position(source_file_id, source_generation, source_offset, window_kind)` 拒绝同一物理 JSONL 位置/window 产生两个事实。Session 删除只把 observation 的 Session 外键置空，长期审计 history、source lineage 与 receipt 保留。
...
- `job_runs(job_id, job_type, requested_by, priority, state, phase, source_file_id, resume_of_job_id, created_at_ms, started_at_ms, finished_at_ms, progress_current, progress_total, resume_generation, resume_offset, error_class, updated_at_ms, resume_consumed_by_job_id)`
...
- `process_snapshots(snapshot_id, session_id, pid, cpu, rss, ports_json, child_count, captured_at_ms, valid_until_ms)`（仅设计，当前未建表）
- `app_runtime_samples(captured_at_ms, cpu_percent, cpu_user_ms, cpu_system_ms, rss_bytes, peak_rss_bytes, goroutine_count, db_bytes, wal_bytes, disk_free_bytes, live_queue_depth, backfill_queue_depth, oldest_live_wait_ms, oldest_backfill_wait_ms, query_count, query_total_micros, query_max_micros, collector_duration_micros, dropped_samples)`
...
Job state 只允许 `queued`、`running`、`succeeded`、`failed`、`cancelled`、`interrupted`，phase 只允许 `discover`、`fast_bootstrap`、`history_backfill`、`reconcile`、`live`、`maintenance`。Repository 允许 `queued -> running/cancelled/interrupted` 和 `running -> running/succeeded/failed/cancelled/interrupted`；phase、progress 和 update time 不能倒退，terminal row 不可复活。应用重启时把遗留 queued/running jobs 原子标为 `interrupted`；恢复只能经 typed resume API 新建 queued job，`resume_of_job_id` 必须指向 recoverable history，新 job 完整继承 type、priority、source、phase、progress 与 typed `JobCursor{Generation, Offset}`，且 created/updated time 不早于旧 terminal ordering key。第一次消费 interrupted parent 时，同一 writer transaction 会把新 job ID 写入 parent 的 `resume_consumed_by_job_id`；该稳定 identity 不设外键，因而 resume child 在 retention 后删除也不会让 parent 重新变成 recoverable。相同 child ID replay 幂等，不同 child 的二次消费 fail closed。v13 会按已有 reverse lineage 回填该标记。公共 `CreateJobRun` 拒绝直接写 resume lineage。
...
Health event 用唯一 64 位小写 SHA-256 `fingerprint` 折叠同类问题，domain 只能是 `source/job/store/pricing/runtime`。code 不是开放字符串：Source 允许 `timeout/unavailable/permission/corrupt/stale/auth_required/failure_streak`，Job 允许 `interrupted/failed/cancelled/live_queue_stalled/backfill_stalled`，Store 允许 `busy/disk_full/read_only/permission/io/corrupt/unavailable/unknown/disk_low/wal_pressure`，Pricing 允许 `unavailable/invalid`，Runtime 允许 `unknown/cpu_pressure/memory_pressure/metrics_stale/updater_unavailable/updater_unknown`，实际值均带对应 domain 前缀。Repository 与 STRICT DDL 共同校验完整 domain/code pair。完全相同的 observation time/payload 重放 no-op；更晚观测增加 `occurrence_count` 并重开已解决事件；同时间冲突或更早观测会被拒绝。resolve time 不得早于 last seen，且同一生命周期的 resolve 只能完全一致重放；已解决事件只接受严格晚于 `resolved_at_ms` 的观测重开，resolve 之前迟到的 observation 不得回退 `updated_at_ms`。`source_file_id` 与 `job_id` 是可选关联，删除引用对象时置空而不删除事件。
...
没有原生 ID 的结构化观测使用稳定来源 identity、generation、line start offset 与 event kind 组成幂等键。quota observation 对 `(source_file_id, session_id, source_generation, line_start_offset, window_kind)` 的 canonical tuple 做 SHA-256，输出 `quota-local-jsonl-<digest>`；key 不包含文件路径、原始 JSONL 或用户内容 hash。每个 sample 的 typed fact 另做 content-free SHA-256 receipt，用于 coalesce 后的 exact replay 验证。
...
读取完整行并识别结构化字段
    ...
    -> UPSERT sessions / turns / usage / quota observations / current projections
...
TOO-249 已把连接与 Repository 收敛为 GORM-first Pure Go adapter。依赖固定为 `gorm.io/gorm v1.31.2`、`github.com/libtnb/sqlite v1.2.0` 和 `modernc.org/sqlite v1.53.0`；生产 SQLite 编译链不使用 `gorm.io/driver/sqlite` 或 `github.com/mattn/go-sqlite3`，`internal/store/sqlite` 与 `internal/store` 必须在 `CGO_ENABLED=0` 下构建和测试。固定版本 GORM 的上游 `go.mod` 自带 official SQLite driver/mattn 间接元数据，但 `CGO_ENABLED=0 go list -deps ./internal/store` 的实际编译依赖不得包含二者。Helper 的 transport 和业务 runtime 不改变这条 Pure Go SQLite 门禁。

Helper 启动时由 `internal/helper.Run -> internal/app.Open` 打开一个 Store，Store 内部持有一个物理 writer connection 和独立 read-only pool；业务 repository 不得自行打开第二条写路径。GORM models 只存在于 persistence adapter，显式映射到 `Project`、`Session`、`Turn`、`SourceFile`、`JobRun`、`HealthEvent`、`PricingVersion` 等 domain types，不向 RPC contract 或业务层泄漏。
...
应用打开 Store 后、将其暴露给任何 runtime reader/writer 之前，由显式 migration catalog 管理 `PRAGMA user_version` 与 `schema_migrations(version, name, checksum, applied_at_ms)` append-only ledger。`MigrateApplicationSchema` 只属于该启动期 bootstrap 契约；若未来需要运行期 maintenance migration，必须先由上层实现任务排空和 Store 独占，不得直接复用启动调用。catalog 必须从 1 连续、名称和 SHA-256 checksum 稳定；history 缺号、checksum drift、ledger/user_version 分叉或数据库版本高于二进制都会 fail closed。所有 pending migration 在同一个 writer transaction 中顺序执行，只有全部 schema readback 和 ledger 写入成功后才推进 `user_version`，任一步失败会连同本轮所有 pending objects 一起回滚。当前 v1 固定为初始 core/runtime schema，v2 只追加 retention query indexes，v3 增加 durable incremental-ingest checkpoint/generation 并重建 turn 外键闭包，v4 增加 Session/Turn project/model attribution，v5 增加 pricing metadata、cost generation、turn cost 与 session/global/project/model rollup，v6 增加首次索引 bootstrap job/plan、阶段进度、readiness 与多 pass reconcile 状态，v7 增加 live/backfill scheduler，v8 增加 scheduler lifecycle/retry，v9 增加 local JSONL quota observations，v10 增加 online source failure metrics，v11 增加 quota window arbitration projection 并回填已有 observation，v12 增加 reset credits 与 source refresh scheduling，v13 增加 content-free app runtime samples、interrupted resume-consumed marker 与既有 lineage 回填，v14 只重建既有 `health_events` 以扩展有限 code CHECK，v15 只新增 quota projection 的过滤/排序性能索引，v16 增加 lightweight Session/Token index，v17 增加 lightweight model attribution，v18 重算 project identity rule v2 并失效旧 cost rollup，v19 为 quota observation 增加可空展示名称，v20 增加 lightweight Tool/Skill invocation facts，v21 为年度 token-only 汇总增加覆盖索引，v31 为 lightweight scanner 增加 last-raw counter/presence/`counter_epoch`，v32 增加 Codex account binding、HMAC scope 目录，并把在线 quota/reset/schedule/claim 绑定到 `(account_scope, binding_generation)`，v33 增加 Codex 订阅目录，v34 增加 legacy quota history 的显式关联和单调 generation。fresh、历史升级、固定 checksum 和失败回滚均有测试；新增 migration 不得重算或改写既有 history，也不得把 `account_scope=default` 自动归属到新账号。

### Lightweight Tool / Skill 调用事实

schema v20 在 lightweight generation 下增加 `light_invocation_events`。它只保存 `(session_id, generation, source_offset, event_ordinal)`、观测时间、`tool|skill`、安全名称、有限来源、有限结果和可空耗时；查询只连接当前 active generation。parser version 升级为 `codex-token-model-invocation-v3`，让已有 rollout 通过受控 rebuild 补齐该投影，旧 generation 在新 generation 完整激活前继续可见。

schema v21 只增加 `idx_light_token_timed_usage_summary`，覆盖观测时间、active generation 连接键和四类 token 数值，不新增或改写事实。年度 token-only 查询由 Go 按 IANA 时区生成本地自然日半开区间，SQLite 使用该索引按日求和；完整 Usage 查询仍保留历史价格版本与模型维度，不复用 token-only 降维结果。

schema v31 为 `light_token_scans` 增加可空 last-raw counter、presence 标志和 `counter_epoch`。`input_tokens` 等四列继续是持久化 aggregate totals（各 epoch 正增量之和），不得把原始累计快照误当总量。presence=0 且 raw 为 `NULL` 表示字段缺失；presence=1 且 raw=0 才是真实零。parser version 升级为 `codex-token-model-invocation-v4`，让历史 high-water 漏计通过受控 rebuild 纠正；旧 active generation 在新 pending generation 完整激活前继续可读。pending generation 对文件增长、incomplete tail、truncate/prefix drift/parser change 复用 `DecideRefresh`：safe append 只 CAS 更新 pending source identity 并从 durable offset 继续；不安全变化原子 supersede/restart pending。单个 Session 的可恢复 scan conflict/error 不得饿死后续 Session；Home identity fence、取消、数据库损坏或无法保证一致性的全局错误仍必须终止。

schema v32 增加 Codex 当前账号隔离所需的 binding 表，并让在线 schedule/claim/attempt 携带同一 `(account_scope, binding_generation)`：

| 表 | 责任 |
| --- | --- |
| `codex_account_scope_key` | 单行 32 字节安装级随机密钥；只用于内存中派生 HMAC scope。 |
| `codex_account_scopes` | 已见 64 位 hex scope 的 first/last seen；A→B→A 复用同一 scope 行。 |
| `codex_account_binding` | 单行当前 state、scope、generation、reason；非 confirmed 时 `account_scope` 为空。 |
| `source_refresh_global_fences` | App Server 在线组的 manual/backoff 下限，不再按 legacy `quota:wham:default` 身份调度当前账号。 |
...
schema v34（`codex-quota-history-association`）为 legacy quota history 增加两张 STRICT 表，不修改 v1–v33：

| 表 | 职责 |
| --- | --- |
| `codex_quota_history_association_generations` | 保留单调 revision，防止撤销再关联后的旧请求命中新关联。 |
| `codex_quota_history_associations` | 将唯一 legacy scope `default` 显式、可撤销地关联到一个 HMAC account scope。 |

v34 只建立关联元数据；不复制、不更新、不删除 `quota_observations`。checksum 固定为 `74e453f11f46fb98dc052c1d7ac77ce80b4b103a5963ea7fd8f8a032c3c9784e`。
...
fresh 空库不做无意义备份。已有用户 schema 且存在 pending migration 时，runner 先只读检查状态和可用空间，再通过 modernc `NewBackup` / `Step` / `Remaining` / `PageCount` 创建包含 committed WAL 页的 `0600` 快照；临时快照以只读连接执行固定 `PRAGMA quick_check`，只有返回 `ok`、fsync 并原子发布后才进入 migration transaction，失败或取消只清理 `.partial-*`。文件级恢复原语使用 `NewRestore`，只恢复到尚不存在的新文件，不在运行中覆盖当前 Store。`STRICT`、复杂 `CHECK`、特殊 index、连接 PRAGMA、`sqlite_schema` canonical readback、`quick_check` 和 `EXPLAIN QUERY PLAN` 是隔离的 raw SQL 例外；普通 CRUD、filter、关联检查和事务编排使用 GORM。

TOO-295 将上述原语接入 Helper startup gate。migration failure 关闭当前 Store 后只保留 content-free recovery snapshot；普通 Store、Repository 与 application service graph 不启动。restore 选择仅限私有 `0700` backup 目录内经 `O_NOFOLLOW` 打开的 regular `0600` DB，prepare 时冻结 SHA-256，confirm 时从新 descriptor 复制并复核摘要；恢复到 working DB 后重新执行 v1-v18 migration、完整 schema verify 和 builtin pricing readback，再用 modernc Backup 固化 ready DB。canonical 切换前以 SQLite Online Backup 保留包含 committed WAL 的失败库，随后用 Darwin 原子文件交换发布 ready DB；pre-commit 同步失败原子回滚，post-commit 同步异常显式返回 committed warning，因此不会覆盖唯一副本或留下 canonical path 缺口。恢复成功仍需新进程再次通过 startup gate。
...
- `quota_observations(account_scope, source, source_file_id, window_kind, limit_id, last_observed_at_ms DESC, observation_id DESC)` 支持后续逐窗口 current 候选读取；partial unique physical-source position index 保护 local JSONL 幂等位置；`quota_observation_receipts(segment_observation_id, observation_id)` 支持 segment 级 receipt 清理与审计。
- `app_runtime_samples(captured_at_ms)` 支持 runtime series 半开窗口、倒序上限读取与后续 24 小时 retention；主键显式关闭 GORM auto increment，保证合法时间零值不会被改写。统一 metrics snapshot 只接受精确 24 小时窗口，完整返回 Detailed 5 秒 cadence 的 17,280 个点，超过容量显式 fail closed；当前 recoverable interrupted job 只认 `resume_consumed_by_job_id IS NULL`，不依赖可能被 retention 删除的 child row。普通写入/readback/聚合使用 GORM，只有 `STRICT/CHECK/index` migration DDL 保留为隔离的 raw SQL 例外。
...
上述 required query 由 GORM model/scopes 构造；测试文件维护等价的代表查询用于 `EXPLAIN QUERY PLAN`，要求命名索引被选择且不得出现临时排序。process snapshot 仍因缺少可信 session→PID 映射而未创建。core/runtime 表不提供 prompt、response、tool output、原始 JSONL、鉴权 token 或 raw error 的专用字段；schema contract 和数据库 bytes 测试会对受控 code/digest/cursor/error 输入面使用 synthetic marker 验证拒绝与不可见性。业务 identity/path metadata 的凭据卫生仍由调用方负责。
...
`queued`、`running` 与 `interrupted` job 都不清理；`interrupted` 是可恢复或已消费终态，不能因已有 `finished_at_ms` 被误当成普通已完成历史。`resume_consumed_by_job_id` 永久保留一次性消费事实，child row 清理后也不清空。未解决 health event 永久越过当前 cleanup；已解决但仍在窗口内的 event 继续阻止关联 job 删除。任一 resume child 仍引用 parent 时，parent 也保留，不能依赖 `ON DELETE SET NULL` 抹掉解释和恢复血缘。

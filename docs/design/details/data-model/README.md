...
application schema v9 创建 `quota_observations` 与 `quota_observation_receipts`，v10 为 online source state/attempt 追加 typed failure metrics，v11 创建可重建 `quota_current` 与逐 observation 的 `quota_arbitration_evidence`，v19 为 observation 追加 nullable `limit_name`。v0.1 的 `account_scope` 固定为 `default`。新写入的 local JSONL observation 必须带 Session 和 durable `source_file_id` 且不得带 request；Session 后续删除时可由外键置空，但 physical source lineage 保留。online observation 必须带 request、不得带 source file，Session 可空。`limit_id` 负责稳定区分通用与模型专属额度，`limit_name` 只负责展示，不能反向替代 identity。

...
wake、显式 resume 与来源恢复不会直接开放 worker，而是先进入 `reconciling`，在前后两次核对 confirmed Home 的 generation、canonical path、device 与 inode，并执行轻量 reconcile；added/grown/truncated/moved/replaced action 使用稳定 identity 进入既有 live runtime 与 durable scheduler queue，deleted/unreadable 仍交给 bootstrap/reconcile contract。任一来源不可用或 generation 漂移都持久化 `blocked + unavailable`。用户 pause 与 system sleep 正交，因此 wake 不清除用户暂停；重复 pause/wake/source event 精确重放时不增加 revision、也不重复 reconcile。应用启动在打开 SQLite 后真实装配 bootstrap/live executor、scheduler worker、coordinator 与 Wails adapter，先完成 startup recover/reconcile，再让 worker 持有 owner lease；退出按 adapter、worker、coordinator、SQLite 的顺序关闭。启动看到遗留 `draining/reconciling` 会完成对应步骤；`pause=all` 时不恢复 active task，避免启动绕过用户意图。

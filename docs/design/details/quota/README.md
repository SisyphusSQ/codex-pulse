...
- snapshot：`limit_id`、`limit_name`、`primary`、`secondary`、`plan_type`；
- window：`used_percent`、`window_minutes`、`resets_at`；
- envelope：记录时间与 JSONL 行的 source offset。

`limit_name` 作为可选展示名进入 observation；缺失时保留 `NULL`，不得从窗口位置猜模型。credits、future fields 和其它未知字段继续忽略，不进入 parser event、diagnostic、checkpoint 或数据库。`remaining_percent` 不持久化，也不从 token 用量猜测；它只能由后续展示层从 `100 - used_percent` 派生。
...
Wham decoder 先在有界内存中递归拒绝任意层级 duplicate JSON key，再读取 `plan_type`、默认 `rate_limit.primary_window/secondary_window` 和有界的 `additional_rate_limits[]`。默认桶固定使用 `limit_id=codex` 且名称为空；每个附加桶必须同时提供唯一 `metered_feature` 作为稳定 `limit_id`、`limit_name` 作为展示名，以及可选的 `rate_limit`。附加桶的 `rate_limit` 缺失或为 `null` 表示当前没有窗口，可安全忽略；缺失/超长身份、重复 `metered_feature` 或已出现但损坏的 `rate_limit` 只隔离该桶，并让本次请求返回 `schema_incompatible`，不会覆盖已解析的默认桶。未知附加字段忽略；未知但类型正确的未来 plan 归一为 `unknown + suspicious/unknown_plan_type`，不会生成零值。每个有窗口的桶内 primary 必须合法，secondary 可以缺失；secondary 损坏时保留合法 primary 并同时返回 `schema_incompatible`，primary 缺失但 secondary 合法时保留 secondary 为 `suspicious/missing_primary_window`。window 字段继续遵守 `used_percent=0..100`、最大 525600 分钟、epoch overflow 和 reset 可信度规则；Wham observation ID 同时绑定 request、limit ID、window kind 与观测时间，多个同周期额度不会冲突。

application schema v10 通过 GORM Migrator 为 `source_state` 和 `source_attempts` 追加 exact failure code、attempt count、response byte count 与 retry time；schema v19 为 `quota_observations` 追加 nullable `limit_name`，既有 observation 保持 `NULL`，v1～v18 的 schema/checksum 保持冻结。`RecordQuotaFetch` 在现有单 writer transaction 中同时写 observation、append-only attempt 和 source freshness：成功清空失败状态；partial/failure 保留 last-known-good 并进入 stale/unavailable；cancelled 不累计连续失败；较旧请求晚到只补历史，不回退较新的 source state。exact request replay 是 no-op，同 request 不同结构整笔 fail closed。

可复用的 synthetic-only 验证入口见 [`docs/test/wham.md`](../../../test/wham.md)。

### 窗口代际、校验与可信仲裁（TOO-264）

窗口 identity 使用 `(window_kind, limit_id, window_minutes, resets_at_ms)`；逻辑 current key 使用 `(account_scope, window_kind, limit_id)`。通过校验的 `resets_at_ms` 同时作为确定性的 `window_generation` key：projection 从完整 observation history 重算时不会因输入顺序或补入旧历史而重编号，但不假定滑动窗口必须等旧 reset 到期后才能给出更晚的新 reset。同一代际内同一来源的 used 只能保持或上升；较新 observation 给出结构合法、位于自身窗口时长内的更晚 reset 时进入新代际，允许 used 从低值重新开始。
...
- primary 必须存在，字段类型和 observation 时间不能明显倒退；默认允许的系统时钟偏差为 2 分钟，规则版本为 `quota-arbiter-v8`；
...
额度数值与展示名称使用独立选择语义：数值、reset、freshness 继续严格来自 arbiter 选中的 observation；`limit_name` 优先读取该 observation，缺失时可从同一逻辑额度最新的 accepted observation 补全。最新同时间点存在冲突名称时保持名称为空，不任取一个来源；客户端对默认 `limit_id=codex` 回退“通用额度”，对其它无名称额度回退“模型专属额度”，不得把内部 `limit_id` 直接展示给用户。
...
### Core RPC 手动刷新合同（TOO-276）

`RequestQuotaRefresh` Core RPC 只接受有限 `quota` / `reset_credits` source。`internal/core.Service` 在 Helper 启动时绑定既有 `applicationLifecycleRuntime` command；它不重写 coordinator，不绕过 60 秒 durable 手动间隔、Retry-After、active claim、generation fence 或调用期 credential provider。返回 receipt 只包含 source、next due、固定 reason 与 last manual time，不暴露 source instance、scope、claim ID、revision、token 或底层 cause；非法 source 与依赖失败映射为 content-free gRPC status 与 `ErrorDetail`。

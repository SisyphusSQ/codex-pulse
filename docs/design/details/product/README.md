...
三项快捷按钮都必须进入稳定的原生键盘焦点顺序，支持 Tab 导航和 Return / Space 激活，并提供明确的无障碍标签。截图渲染不可用、系统剪贴板写入失败或外部 URL 打开失败时必须显示用户可见错误；失败路径不得回退复制原始 Session、JSONL、完整账号标识、鉴权材料、日志或其它敏感数据，也不得在只写入其中一种剪贴板表示后宣称成功。

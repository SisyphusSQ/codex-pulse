...
- 总量与按客户端堆叠的 Token 趋势，以及带客户端命名空间的模型 Top N。
- “工具与模型用量”用加粗的横向构成条同时表达客户端份额和客户端内模型构成；“工具分布”“模型分布”用环图表达 Token / 费用占比，超出模型 Top N 的已知余量归入“其他”。
- API 等价估算成本与 Provider reported charge 始终分开。

趋势、热力图、构成条和环图都提供 pointer hover 详情；卡片提供进入对应客户端概览的可访问操作。不做跨客户端 Session 列表、项目排名或全局 Input/Output/Cache 三张汇总卡。

### 概览

各客户端“概览”由原 Usage 页面升级而来，按“先看额度、再看趋势和成本”排序：

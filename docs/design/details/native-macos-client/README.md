...
当前 Session 自适应趋势属于破坏性语义演进：该能力随 `core-rpc-v2` 引入，`Contracts.usage_cost_version` 为 `usage-cost-v2`。旧
...
- 账户信息属于 Go 拥有的本地业务能力：Codex 路径在已确认的 canonical Home 下启动受控 App Server。邮箱和套餐只能通过同一会话夹读 `account/rateLimits/read → account/read → account/rateLimits/read` 发布；前后 `accountId` 派生出的 scope 必须一致且匹配当前 binding generation。Pro 档位只使用夹读得到的一致 `planType`：`prolite` 映射为 `Pro 5×`，`pro` 映射为 `Pro 20×`；缺失、未知或冲突证据显示 `Pro · 档位未知`。额度百分比、Token、窗口、reset 时间、Reset Credits 不得用来推断档位。Swift 用 `CodexAccountContextKey(scope, generation)` 校验 quota、pace 与 account：只有 `state==confirmed`、scope 非空、generation > 0 时才产生 key。首次组装 Overview 也必须校验，禁止发布“B 额度 + previousAccount A”。pending / signed_out / identity_unavailable 显示“账号确认中”或 unknown，并清空旧邮箱和旧档位。Cursor 路径只从 Desktop `state.vscdb` 白名单读取 `cachedEmail`、`stripeMembershipType` 和订阅状态；Grok 路径只从 `auth.json` 白名单读取可展示邮箱与身份类型，套餐名来自 billing 的 `subscriptionTier`。三者都由带 `ProviderScope` 的 `AccountSnapshot` Core RPC 传给 Swift，Swift 不直接读取 SQLite、JSONL 或 session 文件。邮箱和套餐只在正常 Popover 账户胶囊中展示；复制完整截图时必须隐藏二者，剪贴板文本表示也不得包含这些值、`Pro 5×` / `Pro 20×`、原始 plan token、scope 或 generation。token、refresh token、cookie、授权头、原始 `accountId`、未声明的原始响应字段和原始 session 内容不得进入 DTO、日志或持久化。Codex 首次读取失败或账号未确认时账户胶囊和在线额度都显示 unavailable，不得沿用上一账号；Cursor/Grok 后续失败仍可保留上次成功值，不降级整页 Overview。Grok 客户端的其余产品边界见 [Agent Provider、Cursor 与 Grok](../providers/README.md)。

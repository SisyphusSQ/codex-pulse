...
本文件登记 Codex Pulse 当前可执行的项目级机械约束。`enforced` 必须对应真实命令；后续 Swift 客户端、`.app`、签名、公证与 live E2E 尚未实现，不能标记为已验证。
...
| `VERIFY-003` | `verification` | 统一入口必须串联 harness、project、Proto、race、vet、Helper build | `Makefile` | Make targets + CI | `make verify` | `enforced` | `maintenance_candidate` | Swift/.app/live E2E 明确不在本轮 |
...
| `SWIFT-001` | `docs` | Swift client、Helper process supervision、`.app`、签名、公证与 live E2E | 本 ExecPlan | none | - | `planned` | `human_decision_required` | 后续 Swift 任务承接，`cross-repo` 边界届时确认 |

...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `ARCH-001` | `architecture` | 后端使用 Go + Wails3，前端使用 Vue 3 + TypeScript + Vite，本地 SQLite 与 Go 查询层是业务事实和聚合口径的权威来源 | `README.md` | review | - | `documented` | `human_decision_required` | 当前尚未生成应用工程；正式初始化时固定具体依赖版本并提交 lockfile |

...
| `DATA-001` | `contract` | 只读增量索引 Codex 本地 JSONL；不得复制原始 JSONL，也不得持久化完整 prompt、response 或工具输出 | `README.md` / `docs/test/m11-e4.md` | synthetic parser→Store→backup + schema/DTO/artifact contract | `make m11-privacy-audit` | `enforced` | `maintenance_candidate` | `m11-privacy-v1` 用 synthetic canary、43 表 application schema的TEXT/BLOB、generated DTO、frontend cache、可编辑设计源、视觉复核hash manifest、workflow与packaged App regular/symlink机械验证；真实 Home只读闭环由TOO-298独立证明 |
...
| `SEC-001` | `security` | access token、refresh token、Authorization header、Cookie 及其他凭证只能临时进入内存，不得写入数据库、日志或仓库 | `README.md` / `docs/test/m11-e4.md` | memory credential lifecycle + synthetic privacy/artifact contract | `make m11-privacy-audit` | `enforced` | `maintenance_candidate` | quota focused tests验证 request lease结束即清 header与provider close；runner拒绝credential envelope并只输出固定 finding code；在线 quota/reset credits继续可关闭 |

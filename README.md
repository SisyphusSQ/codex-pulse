...
项目已进入 M1 工程基线阶段。当前仓库包含可启动的 Go + Wails3 + Vue 3 空应用壳、generated bindings、前后端测试与 macOS 15 arm64 binary build 入口；SQLite、Codex 索引、quota、Tray、Popover、Updater 和正式页面仍由后续 Execution Issue 实现。
...
当前 `build` 入口只交付 arm64 binary；正式 `.app` bundle、图标与 ad-hoc 签名由 M1-E3 承接，普通开发卡不会触发正式发布。
...
| `build/` | Wails dev/build task、应用 metadata 与 macOS 15 arm64 binary 约束 |
